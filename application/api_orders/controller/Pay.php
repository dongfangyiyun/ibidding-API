...
class Pay extends Rest
{
    ...
    public function pre_pay(Request $request)
    {
        ...
        if ($validate->scene('check')->check($data)) {
            ...
            $result = model('orders')->checkOrder($data['order_id']);
            if ($result['code']) {
    ...
    public function money(Request $request)
    {
        ...
        if ($validate->scene('money')->check($data)) {
            ...
            if ($result['code']) {
                $this->update_goods_info($data['order_no']);
            ...
            if (strpos($order_no, 'N') !== false) {
                ...
                if ($order) {
                    if ($order['is_pay'] == 0) {
                        ...
                        if ($result !== false) {
                            $this->update_goods_info($order_no);
                            echo 'success';
                        }
     ...
     * 【order】处理库存与收藏量
     */
    private function update_goods_info($order_no)
    {
        $order = model('orders')->findOne(['order_no' => $order_no]);
        if ($order) {
            ...
            foreach ($order_goods as $key => $value) {
                ...
                if ($value['spec_group_id']) {
                    model('goods_spec_groups')->updateNum($value['spec_group_id'], '-', 'stock', $num);
                }
            }
        }
    }

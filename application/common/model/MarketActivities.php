...
{
    // 指定数据表
    protected $table = 'ibid_market_activities';
    ...
    public function getRuleInfoAttr($value, $data)
    {
        if ($data['type'] == 'discount') {
            $rules = model('activity_discount_rules')->getAll(['market_activity_id' => $data['id']]);
        } elseif ($data['type'] == 'full') {
            $rules = model('activity_full_rules')->getAll(['market_activity_id' => $data['id']]);
        } elseif ($data['type'] == 'coupon') {
            $rules = model('activity_coupons')->getAll(['market_activity_id' => $data['id']]);
        } else {
            $rules = '';
        }

        $rule_info = [$value => $rules];
        return $rule_info[$data['rule_info']];
    }

    // 获取器【end_timestamp】
    public function getEndTimestampAttr($value, $data)
    {
        $end_timestamp = [$value => strtotime($data['end_time'])];
        return $end_timestamp[$data['end_timestamp']];
    }
    ...
    public function getActivityInfo($user_id, $money, $goods_ids)
    {
        ...
        foreach ($activities as $key => $activity) {
            ...
            if ($activity['type'] == 'full') {
                $full_rules = model('activity_full_rules')->getAllBasicInfo(['market_activity_id' => $activity['id'], 'full_money' => ['<', $money]]);
